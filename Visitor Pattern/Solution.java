...
public class Solution {
    ...
    public static Tree solve() {
        ...
        try {
            ...
            Iterator<Entry<Integer, Set<Integer>>> relationsIter = relationships.entrySet().iterator();

            while (relationsIter.hasNext()) {
                Entry<Integer, Set<Integer>> currRelation = relationsIter.next();
                StringBuilder strRelatedNodes = new StringBuilder();
                Set<Integer> relatedNodes = currRelation.getValue();
                for (int relatedNode : relatedNodes) {
                    if (strRelatedNodes.length() != 0)
                        strRelatedNodes.append(", ");
                    strRelatedNodes.append(relatedNode);
                }
                System.out.printf("%d - %s\n", currRelation.getKey(), strRelatedNodes.toString());
            }
            ...
            Iterator<Entry<Integer, Integer>> depthsIter = depths.entrySet().iterator();

            while (depthsIter.hasNext()) {
                Entry<Integer, Integer> currDepth = depthsIter.next();
                System.out.printf("%d - %d\n", currDepth.getKey(), currDepth.getValue());
            }
